...
public class RestControllerTest {
  ...
  public void sendFriendRequestTest() throws Exception {
    ...
    String idTestUser =
        mockMvc
            .perform(
                MockMvcRequestBuilders.post("/api/v1/users/get-id-by-username")
                    .contentType(MediaType.APPLICATION_JSON)
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.post("/api/v1/users/send-friend-request")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"from_id\":\"" + idTestUser + "\",\"to_id\":\"" + idRobertSmith + "\"}"))
        .andExpect(MockMvcResultMatchers.status().isOk());
  }
  ...
  public void getAllRequestsTest() throws Exception {
    ...
    String response =
        mockMvc
            .perform(
                MockMvcRequestBuilders.get("/api/v1/users/" + idRobertSmith + "/friend-requests"))
            .andExpect(MockMvcResultMatchers.status().isOk())
            .andReturn()
            .getResponse()
            .getContentAsString();
  ...
  public void registerAndSendRequestToRobert() throws Exception {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.post("/api/v1/users/send-friend-request")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"from_id\":\"" + userId + "\",\"to_id\":\"" + roberId + "\"}"))
        .andExpect(MockMvcResultMatchers.status().isOk());
  }
